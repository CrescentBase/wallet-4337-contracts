...
import { arrayify, BytesLike, hexConcat, hexlify, hexZeroPad, keccak256 } from 'ethers/lib/utils'
import { Provider } from '@ethersproject/providers'

export class Create2Factory_EIP2470 {
  ...
  async deploy (initCode: string, salt: BigNumberish, gasLimit?: BigNumberish | 'estimate'): Promise<string> {
    ...
    const factory = new Contract(Create2Factory_EIP2470.contractAddress, ['function deploy(bytes _initCode, bytes32 _salt) returns(address)'], this.signer)
  ...
  async getDeployTransactionCallData (initCode: BytesLike, salt: BigNumberish = 0): Promise<BytesLike> {
    const saltBytes32 = hexZeroPad(hexlify(salt), 32)
    const factory = new Contract(Create2Factory_EIP2470.contractAddress, ['function deploy(bytes _initCode, bytes32 _salt) returns(address)'])
    return await factory.populateTransaction.deploy(initCode, saltBytes32).then(tx => tx.data!)
  }
  ...
  getDeployedAddress (initCode: string, salt: BigNumberish): string {
    ...
    return '0x' + keccak256(hexConcat([
      '0xff',
      Create2Factory_EIP2470.contractAddress,
      saltBytes32,
      keccak256(initCode)
    ])).slice(-40)
  }
  ...
  async deployFactory (signer?: Signer): Promise<void> {
    ...
    await (signer ?? this.signer).sendTransaction({
      to: Create2Factory_EIP2470.factoryDeployer,
      value: BigNumber.from(Create2Factory_EIP2470.factoryDeploymentFee)
    })
    await this.provider.sendTransaction(Create2Factory_EIP2470.factoryTx)
    if (!await this._isFactoryDeployed()) {
  ...
  async _isFactoryDeployed (): Promise<boolean> {
    if (!this.factoryDeployed) {
      const deployed = await this.provider.getCode(Create2Factory_EIP2470.contractAddress)
